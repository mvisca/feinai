...
describe('task ↔ spec status transitions', () => {
  ...
  it('takeTask moves spec to in_progress when it was pending', () => {
    const db = setup();
    addSpec(db, { id: 'SPEC-1', title: 'Test' });
    addTask(db, { id: 'T-1', subject: 'Task', spec_id: 'SPEC-1' });

    takeTask(db, 'T-1', 'agent');
    expect(getSpec(db, 'SPEC-1')!.status).toBe('in_progress');
  });

  it('takeTask does not move spec if it is already in_progress', () => {
    const db = setup();
    ...
    expect(getSpec(db, 'SPEC-2')!.status).toBe('in_progress');
  ...
  it('doneTask moves spec to completed when no pending/in_progress tasks remain', () => {
    const db = setup();
    ...
    expect(getSpec(db, 'SPEC-4')!.status).toBe('completed');
  });
  ...
  it('doneTask does not move spec when other tasks remain pending', () => {
    ...
    expect(getSpec(db, 'SPEC-5')!.status).toBe('in_progress');
  });
  ...
  it('doneTask does not move spec when other tasks remain in_progress', () => {
    ...
    expect(getSpec(db, 'SPEC-6')!.status).toBe('in_progress');
  });
  ...
  it('doneTask does not move spec when tasks are failed', () => {
    ...
    // T-7b still in_progress, so spec stays in_progress
    expect(getSpec(db, 'SPEC-7')!.status).toBe('in_progress');
  });
  ...
  it('doneTask does not move spec if it was already completed', () => {
    const db = setup();
    ...
    expect(getSpec(db, 'SPEC-9')!.status).toBe('completed');

    // Adding a new task and completing it should keep spec as completed
